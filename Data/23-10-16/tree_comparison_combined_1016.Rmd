...
# Read path data
#path_data <- readLines("./MAFFT-Auto-gt/result/UCE-1003/path_info.txt")
...
for (comb in tree_combinations) {
    ...
    if (all_rooted) {
	  geodesic_dist <- tryCatch({
	    round(dist.multiPhylo(c(tree1,tree2), method = "geodesic", outgroup = outgroup_subset, force.multi2di = TRUE), 4)
	  }, error = function(e) {
	    NA
	  })
	} else {
