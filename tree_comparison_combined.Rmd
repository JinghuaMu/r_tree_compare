...
```{r}
load(paste(params$workingdict,"path_info.RData", sep = ""))
library(Biostrings)

if (tools::file_ext(gene_file) == "fasta") {
...
kable_styling(knitr::kable(alignment_data, digits = 4, caption = "Alignment Summary"), bootstrap_options = c("striped", "hover", "condensed"))
```
...
tree_species_trimed <- drop.tip(Species_tree, drop_taxa)
...
kable_styling(knitr::kable(result_df, digits = 4, caption = "Tree distance Summary"), bootstrap_options = c("striped", "hover", "condensed"))
...
stats_single <- tree_stats(model_sum = sum_one, tree = One_class_tree, model_name = "One-class")
...
kable_styling(knitr::kable(tree_summary, digits = 4, caption = "Branch length Summary"), bootstrap_options = c("striped", "hover", "condensed"))
