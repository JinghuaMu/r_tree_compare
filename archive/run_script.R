...
for (i in start_point:min(length(gene_sets), end_point)){
  ...
  # execute the rmarkdown file
  output_path <- paste(store_path, gene_name,"_summary.html", sep = "")
  save(outgroup, Species_tree_path, gene_file, gene_path, gene_name, prefix_single, prefix_mix, file = paste(store_path, "path_info.RData", sep = ""))
  if (file.exists(output_path)) {
    if (restart) {
      rmarkdown::render("tree_comparison_combined.Rmd", params = list(workingdict = store_path), output_file = output_path)
    }} else {
      rmarkdown::render("tree_comparison_combined.Rmd", params = list(workingdict = store_path), output_file = output_path)
  }
