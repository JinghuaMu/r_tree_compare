summarise_iqtree <- function(file_path) {
    ...
    if (grepl("(m|M)ixture", model_info[1])){
      ...
      line_SPR <- grep("Site proportion and rates", model_info)
      ...
      if (line_SPR){
        block_SPR <- model_info[(line_SPR+2):length(model_info)]
        iqtree_summarise$RHAS <- read.table(text = block_SPR, header = TRUE)
      } else { iqtree_summarise$RHAS <- NULL}
      ...
      if (grepl("equal frequencies", model_info[line_F])){
        ...
        parameter <- paste0(model_components[1], "{", paste(as.character(iqtree_summarise$R), collapse = ","),  "}+FQ")
      } else {
      ...
      rm(line_sub, line_R, line_F, line_Q, line_SPR, line_RHAS)
      rm(block_R, block_Q)
